...
export function DebtSettlementSimulationClient({ debts }: DebtSettlementSimulationClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-border/80 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Dívidas e opções</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {debts.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border/80 px-6 py-10 text-sm text-muted-foreground">
              Nenhuma dívida elegível para o simulador no momento.
            </div>
          ) : (
            debts.map((debt) => {
              const selectedOptionId = selectedOptionIdsByDebtId[debt.id];
              const selectableOptions = debt.settlementOptions.filter(
                (option) => option.status === "active" || option.status === "accepted"
              );
              ...
              return (
                <div key={debt.id} className="rounded-xl border border-border/80 bg-background p-4 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    {selectedOptionId ? (
                      <Button type="button" variant="ghost" size="sm" onClick={() => removeSelection(debt.id)}>
                        Limpar seleção
                      </Button>
                    ) : null}
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    {debt.settlementOptions.map((option) => {
                      const isExpired = option.status === "expired";
                      const isSelected = selectedOptionId === option.id;
                      ...
                      return (
                        ...
                        >
                          <div className="flex w-full items-center justify-between gap-2">
                            <span className="font-medium">{getOptionTitle(option.kind, option.installments, option.monthlyInstallmentCents)}</span>
                            <Badge variant={getStatusBadgeVariant(option.status)} className="rounded-full">
                              {getStatusLabel(option.status)}
                            </Badge>
                          </div>
                  ...
                  {selectableOptions.length === 0 ? (
                    <p className="mt-3 text-sm text-muted-foreground">
                      Esta dívida só possui opções vencidas no momento.
                    </p>
                  ) : null}
                </div>
              );
